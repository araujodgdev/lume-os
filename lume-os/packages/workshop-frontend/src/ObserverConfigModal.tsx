...
export default function ObserverConfigModal({
  ...
  useEffect(() => {
    ...
    subscription.catch(err => {
      ...
      toasts.add({ title: 'Não foi possível carregar suas contas conectadas', variant: 'error' })
    })
  ...
  const handleReconnect = async (accountId: number) => {
    ...
    try {
      ...
      toasts.add({ title: 'Não foi possível iniciar a nova autenticação', variant: 'error' })
    ...
    <Dialog.Root open disablePointerDismissal onOpenChange={open => { if (!open) onCancel() }}>
      <Dialog className="p-6" size="lg">
        <Dialog.Title className="mb-2 text-lg font-semibold">
          {isRetry ? 'Confirme seu acesso de novo' : 'Confirme seu acesso'}
        </Dialog.Title>
        <Text variant="secondary" size="sm" as="p">
          {isRetry
            ? 'Não foi possível confirmar seu acesso a tudo o que este espaço leu. Autentique de novo ' +
              'a conta abaixo, ou escolha outra, e tente de novo.'
            : 'Antes de abrir este espaço, confirme que as suas próprias contas acessam os dados conectados ' +
              'que ele usa.'}
        </Text>
                ...
                <div key={need.gatekeeperId} className="rounded-xl border border-kumo-line bg-kumo-base p-4">
                  <div className={`flex items-center gap-3${matching.length === 0 && !need.failure ? '' : ' mb-3'}`}>
                    ...
                    {matching.length === 0 && vendor && (
                      ...
                      >
                        {connecting === need.vendorId ? 'Aguardando a conexão…' : 'Conectar'}
                      </WorkshopButton>
                    )}
                  </div>
                    ...
                    <div className="flex flex-col gap-2">
                      {matching.length === 1 ? (
                        <div className="flex min-h-10 items-center gap-3 rounded-lg border border-kumo-line bg-kumo-elevated/50 px-3 py-2">
                          <div className="min-w-0 flex-1">
                            <div className="text-[11px] leading-4 text-kumo-subtle">Usando a sua conta</div>
                        ...
                        <Select
                          ...
                          placeholder={`Escolha uma conta de ${vendorName}…`}
                          onValueChange={v =>
                      ...
                      {chosen && missing.length > 0 && (
                        ...
                        >
                          ...
                          {granting === chosen.id
                            ? 'Aguardando o acesso…'
                            : 'Conceda o acesso necessário para verificar este recurso'}
                        </button>
                      )}
                      ...
                      {chosen && missing.length === 0 &&
                        ...
                        >
                          ...
                          {reconnecting === chosen.id
                            ? 'Autenticando de novo…'
                            : chosen.credentialsValid
                              ? 'Clique para autenticar esta conta de novo'
                              : 'Esta conta expirou; clique para autenticar de novo'}
                        </button>
                      )}
                      ...
                      {!vendor?.autoProvisionsAccount && (
                        ...
                        >
                          <Plus size={11} />
                          {connecting === need.vendorId ? 'Aguardando a conexão…' : 'Conectar outra conta'}
                        </button>
                      )}
                    </div>
                  )}
        ...
        <div className="flex justify-end gap-2 mt-6">
          ...
          >
            {isRetry ? 'Verificar de novo' : 'Verificar e abrir'}
          </WorkshopButton>
        </div>
      </Dialog>
    </Dialog.Root>
