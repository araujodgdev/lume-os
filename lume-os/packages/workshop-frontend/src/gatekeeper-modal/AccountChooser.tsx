    ...
    <section className="overflow-hidden rounded-xl border border-kumo-line bg-kumo-base">
      <div className="border-b border-kumo-line px-3 py-2.5">
        ...
        <p className="mt-0.5 text-[12px] leading-4 font-normal tracking-[-0.2px] text-kumo-subtle">
          {isEmailMailbox
            ? 'Ative a conta receptora de e-mail e escolha o nome da caixa abaixo.'
            : `Escolha qual identidade de ${vendorName} ${resourceTitle ? `o recurso ${resourceTitle}` : 'esta conexão'} deve usar.`}
        </p>
      </div>
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className={`truncate text-[12px] leading-4 font-normal tracking-[-0.2px] ${needsAccess ? 'text-kumo-brand' : 'text-kumo-subtle'}`}>
                    {expired
                      ...
                      : needsAccess
                      ? 'Precisa de permissão adicional'
                      : resourceTitle ? `Conta de ${vendorName} conectada` : 'Conectada'}
                  </p>
                </div>
              </button>
              {expired ? (
        ...
        {(!isEmailMailbox || accounts.length === 0) && (
          ...
          >
            ...
            {isEmailMailbox
              ...
              : accounts.length === 0 ? `Conectar ${vendorName}` : `Usar outra conta de ${vendorName}`}
          </button>
        )}
      </div>
    </section>
