    ...
    body = (
      <div className="rounded-2xl border border-kumo-line/80 bg-kumo-base px-3 py-2.5">
        <p className="text-[12px] leading-[16px] tracking-[-0.15px] text-kumo-subtle">
          {role ? (
            <>Quem tiver acesso de <span className="font-medium text-kumo-default">{roleLabel(role)}</span> precisa</>
          ) : 'Os destinatários precisam'} comprovar que a própria conta alcança:
        </p>
...
export default function ShareModal({ open, onClose, overseer, metadata, currentUser, authenticatedApi }: Props) {
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      toasts.add({ title: 'Não foi possível carregar o compartilhamento', variant: 'error' })
      return null
    }
  }, [overseer])
  ...
  if (requirementsFailed) {
    recipientVerification = (
      <RecipientVerification
        ...
        heading="Verificação dos destinatários"
      />
    )
        ...
        recipientVerification = (
          <>
            <RecipientVerification
              ...
              heading="Verificação do convite direto"
            />
            <RecipientVerification
  ...
  const describeAccess = (info: CollaboratorInfo): string => {
    if (info.addedBy.length > 1) return `Acesso por ${info.addedBy.length} origens`
    ...
    return key?.note ? `Entrou por “${key.note}”` : 'Entrou por um link de compartilhamento'
  }
  ...
  const copyNewLink = async () => {
    ...
    if (copied) {
      ...
      toasts.add({ title: 'Não foi possível copiar o link.', variant: 'error' })
    }
  }
  ...
  const copyWorkspaceUrl = async () => {
    if (await copyToClipboard(workspaceUrl)) {
      ...
      toasts.add({ title: 'Não foi possível copiar o link do espaço.', variant: 'error' })
    }
  }
  ...
  const handleAddCollaborator = async () => {
    ...
    try {
      ...
      if (result === null) {
        toasts.add({ title: 'Nenhuma conta encontrada com esse usuário.', variant: 'error' })
      } else {
        ...
        toasts.add({ title: `${result.profile.name} foi adicionado como colaborador.`, variant: 'success' })
      }
    } catch (err: any) {
      toasts.add({ title: err.message || 'Não foi possível adicionar o colaborador.', variant: 'error' })
    } finally {
  ...
  const handleCreateShareLink = async () => {
    ...
    try {
      ...
      toasts.add({ title: err.message || 'Não foi possível criar o link.', variant: 'error' })
    } finally {
  ...
  const handleCopyShareLink = async (linkId: string) => {
    ...
    try {
      ...
      toasts.add({ title: 'Link copiado.', variant: 'success' })
    } catch (err: any) {
      toasts.add({ title: err.message || 'Não foi possível copiar o link.', variant: 'error' })
    } finally {
  ...
  const handleStartRemoveCollaborator = async (profileId: string) => {
    ...
    try {
      ...
      toasts.add({ title: err.message || 'Não foi possível prever a remoção do colaborador.', variant: 'error' })
    }
  }
    ...
    try {
      ...
      toasts.add({
        title: removed.length > 0
          ...
          : 'Seu acesso direto foi removido. Essa pessoa ainda tem acesso por outra origem.',
        variant: 'success',
      })
      await loadData()
    } catch (err: any) {
      toasts.add({ title: err.message || 'Não foi possível remover o colaborador.', variant: 'error' })
    } finally {
  ...
  const handleSaveShareLinkNote = async () => {
    ...
    try {
      ...
      await loadData()
      showLandedRow('shareLink', linkId)
      toasts.add({ title: 'Link renomeado.', variant: 'success' })
    } catch (err: any) {
      toasts.add({ title: err.message || 'Não foi possível renomear o link.', variant: 'error' })
    } finally {
  ...
  const handleStartRevokeShareLink = async (linkId: string) => {
    ...
    try {
      ...
      toasts.add({ title: err.message || 'Não foi possível prever a revogação do link.', variant: 'error' })
    }
  }
  ...
  const handleConfirmRevokeShareLink = async () => {
    ...
    try {
      ...
      toasts.add({ title: 'Link revogado.', variant: 'success' })
      await loadData()
    } catch (err: any) {
      toasts.add({ title: err.message || 'Não foi possível revogar o link.', variant: 'error' })
    } finally {
      setConfirmationBusy(false)
    }
  }
          ...
          {sharingProhibited ? (
            <div className="flex h-full flex-col items-center justify-center px-6 py-12 text-center">
              ...
              <p className="mt-3 text-[14px] leading-5 font-medium tracking-[-0.3px] text-kumo-default">
                Este espaço não pode ser compartilhado
              </p>
              ...
              <div className="min-w-[160px] flex-1">
                <div className="flex items-baseline gap-1.5">
                  <p className="text-[13px] leading-[18px] font-medium text-kumo-default">
                    {invitedName} adicionado
                  </p>
                  <span className="text-[11px] leading-4 text-kumo-inactive">
                    {invitedLinkCopied ? 'Link copiado' : 'Envie este link para a pessoa abrir'}
                  </span>
                </div>
              ...
              newShareLink ? (
                <div className="themed-compact-shadow flex flex-wrap items-center gap-3 rounded-2xl border border-kumo-line/80 bg-kumo-base px-3 py-2.5 share-fade-in">
                    ...
                    <div className="min-w-[160px] flex-1">
                      <div className="flex items-baseline gap-1.5">
                        ...
                        <span className="text-[11px] leading-4 text-kumo-inactive">
                          Você pode copiá-lo de novo quando quiser em Links de compartilhamento
                        </span>
                      </div>
                ...
                <div className="themed-compact-shadow flex h-12 items-center gap-2 overflow-hidden rounded-2xl border border-kumo-line/80 bg-kumo-base p-1.5 pl-3 transition-[border-color,box-shadow] focus-within:border-kumo-fill share-fade-in">
                    ...
                    <input
                      ...
                      placeholder="Dê um nome a este link (opcional)…"
                ...
                return (
                  <div key={key} className={`group ${index > 0 ? 'border-t border-kumo-line/70' : ''} ${landedPersonId === profile.id ? 'share-row-land' : 'transition-colors duration-150 hover:bg-kumo-elevated/50'} px-3 py-2.5`}>
                    <div className="flex items-center gap-3">
                      ...
                      {row.kind === 'owner' ? (
                        <span className="px-2 text-[12px] text-kumo-subtle">Dono</span>
                      ) : isRemoving ? (
                        <InlineConfirm
                          label="Remover"
                    ...
                    {isRemoving && downstreamDependents.length > 0 && (
                      <div className="mt-2.5 share-expand-in">
                        <p className="mb-1.5 text-[12px] leading-4 text-kumo-subtle">
                          {downstreamDependents.length} {downstreamDependents.length === 1 ? 'outra pessoa perde' : 'outras pessoas perdem'} o acesso por {profile.name}. Manter alguém?
                        </p>
                        <DependentKeepList
                  ...
                  return (
                    <div key={sk.linkId} className={`group ${index > 0 ? 'border-t border-kumo-line/70' : ''} ${landedShareLinkId === sk.linkId ? 'share-row-land' : 'transition-colors duration-150 hover:bg-kumo-elevated/50'} px-3 py-2.5`}>
                        ...
                        <div className="min-w-0 flex-1">
                          {isRenaming ? (
                            <input
                              ...
                              placeholder="Dê um nome a este link…"
                            ...
                            <p className="truncate text-[13px] leading-[17px] font-medium tracking-[-0.25px] text-kumo-default">{sk.note || 'Link sem nome'}</p>
                          )}
                        ...
                        {isRenaming ? (
                          ...
                          <>
                            ...
                            <WorkshopIconButton
                              ...
                              aria-label={`Copiar ${sk.note || 'link'}`}
                            ...
                            <WorkshopIconButton
                              ...
                              aria-label={`Renomear ${sk.note || 'link'}`}
                              disabled={confirmationBusy}
                            >
                            ...
                            <WorkshopIconButton
                              ...
                              aria-label={`Revogar ${sk.note || 'link'}`}
                              disabled={confirmationBusy}
                            >
                              <Trash size={13} />
                            </WorkshopIconButton>
                      ...
                      {isRevoking && revokeTarget.dependents.length > 0 && (
                        <div className="mt-2.5 share-expand-in">
                          <p className="mb-1.5 text-[12px] leading-4 text-kumo-subtle">
                            {revokeTarget.dependents.length} {revokeTarget.dependents.length === 1 ? 'pessoa perde' : 'pessoas perdem'} o acesso por este link. Manter alguém?
                          </p>
                          <DependentKeepList
