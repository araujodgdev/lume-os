  ...
  useEffect(() => {
    const loadVendors = async () => {
      ...
      try {
        ...
        toasts.add({ title: 'Não foi possível carregar os serviços disponíveis', variant: 'error' })
      } finally {
              ...
              <div key={`${vendor.id}-${resource.urlPattern}`} className={i > 0 ? 'border-t border-kumo-line' : ''}>
                ...
                {vendorAccounts.map(account => {
                    ...
                    >
                      ...
                      {isReconnecting || isGranting ? (
                        ...
                        <span className="flex flex-shrink-0 items-center gap-1">
                          ...
                          <span className="text-[11.5px] leading-4 text-kumo-warning">Expirada; clique para autenticar de novo</span>
                        </span>
                      ) : needsAccess ? (
                        <span className="flex flex-shrink-0 items-center gap-1">
                          <Warning size={12} className="text-kumo-warning" />
                  ...
                  if (searchHasPlaceholders) {
                    return (
                      <Tooltip key={account.id} content="Substitua todos os marcadores da URL antes de escolher uma conta" asChild>
                        {accountRow}
                      </Tooltip>
                    )
                  }
