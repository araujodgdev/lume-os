...
export default function AdminFormatsPanel({
  ...
  return (
    <div className="rounded-xl border border-kumo-line bg-kumo-elevated p-6">
      <h2 className="mb-1 text-[22px] font-normal tracking-[-0.03em] text-kumo-strong">Formatos padrão</h2>
      <p className="mb-5 text-sm text-kumo-subtle">
        Um modelo promovido é oferecido pelo nome (“Criar Documento”, “Criar Apresentação”) onde as
        pessoas começam algo, e o agente passa a preferi-lo em vez de construir o mesmo do zero.
      </p>
  ...
  return (
    <div className="mb-5 rounded-lg border border-dashed border-kumo-line bg-kumo-tint/40 p-4">
      <p className="mb-2.5 text-[11px] font-medium uppercase tracking-[0.06em] text-kumo-inactive">
        O que as pessoas vão ver
      </p>
      {formats.length === 0 ? (
        <p className="text-[13px] italic text-kumo-inactive">
          Nada ainda. As pessoas só vão ver “Criar espaço”.
        </p>
      ) : (
      ...
      <p className="mt-2.5 text-[12px] leading-4 text-kumo-subtle">
        No menu + do campo de mensagem, na paleta de comandos e na página de Arquivos vazia, nesta ordem.
      </p>
    </div>
  )
}
...
function EmptyState() {
  return (
    <div className="mb-5 rounded-lg border border-kumo-line bg-kumo-base px-4 py-5 text-center">
      <p className="text-sm font-medium text-kumo-default">Nenhum formato padrão ainda</p>
      <p className="mx-auto mt-1 max-w-md text-[13px] leading-[18px] text-kumo-subtle">
        Promova um modelo para oferecê-lo pelo nome onde as pessoas começam algo, e para que o
        agente o prefira em vez de construir o mesmo do zero.
      </p>
    </div>
  )
}
        ...
        >
          {format.missing ? (
            <span
              ...
              title="Este modelo não existe mais"
            >
              <Warning size={16} />
            </span>
          ) : (
          ...
          <span className="min-w-0 flex-1">
            <span className="flex items-center gap-2">
              ...
              {format.bundled && <Badge>Embutido</Badge>}
              {!format.enabled && !format.missing && <Badge>Desligado</Badge>}
              {needsNaming && <Badge tone="warn">Precisa de nome</Badge>}
            </span>
            <span className="mt-0.5 block truncate text-xs text-kumo-subtle">
              {format.missing
                ? 'Modelo excluído. Remova esta entrada.'
                : needsNaming
                ? 'Este modelo não declara o que produz. Dê um nome a ele para oferecê-lo.'
                : `${format.blueprintTitle} · aparece em ${format.output!.plural} na página Arquivos`}
            </span>
          </span>
        ...
        <div className="flex flex-col gap-4 border-t border-kumo-line px-3 py-4">
          {format.missing ? (
            <p className="text-[13px] text-kumo-subtle">
              O modelo por trás deste formato foi excluído, então ele não é oferecido a ninguém.
              Remova a entrada.
            </p>
          ) : (
            <>
              <Fieldset
                title="Como ele aparece"
                detail={
                  'Deixe um campo vazio para usar o nome que o modelo declara. ' +
                  (format.bundled
                    ? 'Um modelo embutido pode mudar os nomes declarados quando este ambiente for ' +
                      'atualizado; o que você digitar aqui continua como está. '
                    : '') +
                  'Vale para os arquivos criados daqui em diante; os existentes mantêm o nome com ' +
                  'que foram criados.'
                }
              >
              ...
              <Fieldset
                title="Como o agente escolhe"
                detail="Os formatos padrão aparecem primeiro no catálogo do agente, como a entrada abaixo; a própria descrição do modelo faz a maior parte do trabalho. Adicione uma dica só se o agente precisar saber quando preferir este formato a outro."
              >
                <OverrideField
                  label="Dica"
                  placeholder="ex.: preferir para apresentações a clientes"
              ...
              <div className="flex items-end justify-between gap-4 border-t border-kumo-line pt-3">
                <p className="text-[12px] leading-4 text-kumo-subtle">
                  {(format.enabled
                    ? 'Desligar remove este formato dos menus acima e do catálogo do agente. Os arquivos já criados com ele continuam funcionando. '
                    : 'No momento, oculto dos menus acima e do catálogo do agente. ') +
                    (format.bundled
                      ? 'Ele vem com o ambiente, então continua nesta lista de qualquer forma.'
                      : '')}
                </p>
                {!format.bundled && (
  ...
  return (
    <label className="flex flex-col gap-1">
      <span className="text-[11px] font-medium uppercase tracking-[0.06em] text-kumo-inactive">
        {label}
        {overridden && (
          <span className="ml-1 normal-case tracking-normal text-kumo-subtle">(alterado)</span>
        )}
      </span>
      <Input
        value={draft}
