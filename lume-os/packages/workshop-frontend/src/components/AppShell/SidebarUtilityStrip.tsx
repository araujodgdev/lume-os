...
function ThemeModeButton() {
  ...
  const names: Record<ThemeMode, string> = { system: 'sistema', light: 'claro', dark: 'escuro' }
  const label = themeMode === 'system'
    ? `Tema: sistema (${names[resolvedThemeMode]})`
    : `Tema: ${names[themeMode]}`
  const nextMode = names[nextThemeMode(themeMode)]

  return (
    <Tooltip
      content={`${label}. Mudar para ${nextMode}.`}
      render={(
        <button
          type="button"
          aria-label={`${label}. Mudar para ${nextMode}.`}
  ...
  return (
    <div
      className={[
        ...
        'shrink-0 flex h-[52px] items-center gap-1 border-t border-kumo-line bg-kumo-base px-3',
        collapsed ? 'flex-col justify-center gap-2 px-1.5' : '',
      ].join(' ')}
    >
      <StripLink to="/gatekeepers" label="Conectores">
        <Plug size={15} />
      </StripLink>
