...
export default function SidebarGadgetRow({
  ...
  return (
    <Link
      ...
      title={collapsed ? gadget.title || 'Espaço sem título' : undefined}
    >
      <div
      ...
      {!collapsed && (
        <>
          {renaming ? (
            ...
            <span className="min-w-0 flex-1 truncate">{gadget.title || 'Espaço sem título'}</span>
          )}
      ...
      {collapsed && <span className="sr-only">{gadget.title || 'Espaço sem título'}</span>}
    </Link>
  )
}
