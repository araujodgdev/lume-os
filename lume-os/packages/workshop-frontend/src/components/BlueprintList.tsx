...
export default function BlueprintList() {
    ...
    Promise.all([authenticatedApi.listOwnBlueprints(), authenticatedApi.listLibraryBlueprints()])
      .then(([own, library]) => {
        ...
        const ensure = (id: string): BlueprintItem => {
          let it = map.get(id)
          if (!it) {
            it = { id, title: 'Modelo sem título', description: '', recency: 0, pinned: false, inLibrary: false, isOwn: false }
            map.set(id, it)
          }
          return it
        }
  ...
  const handleBlueprintSelected = useCallback(async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      toasts.add({ title: 'Modelo enviado', variant: 'success' })
      load()
    } catch (err) {
      console.error('Failed to upload blueprint:', err)
      toasts.add({ title: 'Não foi possível enviar o modelo', variant: 'error' })
    } finally {
      setUploading(false)
    }
  ...
  const handleRemoveFromLibrary = async (item: BlueprintItem) => {
    try {
      ...
      toasts.add({ title: 'Não foi possível remover o modelo', variant: 'error' })
    }
  }
      ...
      <div className="chat-panel flex min-h-0 flex-1 flex-col gap-0.5 overflow-y-auto pt-1">
        {loading ? (
          ...
          <div className="py-12 text-center text-sm">
            <p className="text-kumo-danger">Algo deu errado ao carregar seus modelos.</p>
            <button type="button" onClick={load} className="mt-1 text-kumo-brand underline">Tentar de novo</button>
          </div>
        ) : filtered.length === 0 ? (
          search ? (
            <div className="py-12 text-center text-sm text-kumo-inactive">Nenhum modelo encontrado</div>
          ) : (
            <div className="flex flex-col items-center gap-3 px-3 py-16 text-center">
              ...
              <div>
                <p className="text-sm font-medium text-kumo-default">Nenhum modelo ainda</p>
                <p className="mt-1 text-[13px] leading-[18px] text-kumo-subtle">
                  Publique um espaço como modelo ou adicione um em Explorar.
                </p>
              </div>
