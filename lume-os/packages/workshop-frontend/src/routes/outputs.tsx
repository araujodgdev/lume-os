      ...
      <DropdownMenu>
        ...
        <DropdownMenu.Content className={MENU_CONTENT}>
          ...
          <DropdownMenu.Item onClick={onOpenWorkspace} className={MENU_ITEM}>
            <Cube size={13} className="mr-2" /> Abrir espaço
          </DropdownMenu.Item>
          {onRename && (
...
function subtitle(output: OutputSummary): string {
  const parts = [output.workspaceTitle || 'Espaço sem título']
  if (output.owner) parts.push(`Compartilhado por ${output.owner.name}`)
  parts.push(`Espaço ativo ${formatRelativeTime(output.lastActive)}`)
  return parts.join(' · ')
}
...
function OutputProvenance({ owner }: { owner?: OutputSummary['owner'] }) {
  return (
    <span
      ...
      title={owner ? `Em um espaço compartilhado por ${owner.name}` : 'Em um espaço criado por você'}
    >
  ...
  return (
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        ...
        <p className="mt-0.5 truncate text-[12px] leading-4 tracking-[-0.2px] text-kumo-subtle">
          {formatOf(output.output).noun} · {output.workspaceTitle || 'Espaço sem título'}
        </p>
      </div>
          ...
          <div className="flex items-start justify-between gap-4 border-b border-kumo-line px-5 py-4">
            <div className="min-w-0">
              ...
              <Dialog.Description className="mt-1 text-[12px] leading-4 text-kumo-subtle">
                Renomeia o arquivo para todos com acesso a “{output?.workspaceTitle}”.
              </Dialog.Description>
            </div>
...
function OutputsPage() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      if (loadedOnce.current) {
        toastsRef.current.add({ title: "Não foi possível atualizar os arquivos", variant: 'error' })
      } else {
        setLoadError(true)
      }
    })
  ...
  const saveRename = async () => {
    ...
    try {
      ...
      toasts.add({ title: "Não foi possível renomear este arquivo", variant: 'error' })
    } finally {
  ...
  const confirmRemove = async () => {
    ...
    try {
      ...
      toasts.add({ title: "Não foi possível remover este arquivo", variant: 'error' })
    } finally {
      gadget?.[Symbol.dispose]()
      overseer?.[Symbol.dispose]()
      setMutationBusy(false)
  ...
  return (
    <div className="mx-auto flex h-full w-full max-w-5xl flex-col px-6 sm:px-10">
      <PageHeader
        className="mx-3 mb-6"
        label="Arquivos"
        title="Tudo o que foi produzido."
        description="Tudo o que os seus espaços produziram, em um só lugar."
      ...
      <div className="chat-panel min-h-0 flex-1 overflow-y-auto pb-8 pt-1">
        {loading ? (
          ...
          <div className="py-12 text-center text-sm">
            <p className="text-kumo-danger">Algo deu errado ao carregar seus arquivos.</p>
          ...
          <div className="flex flex-col items-center gap-3 px-3 py-20 text-center">
            ...
            <div>
              <p className="text-sm font-medium text-kumo-default">
                {isFiltered ? 'Nenhum arquivo encontrado' : 'Nenhum arquivo ainda'}
              </p>
              <p className="mt-1 text-[13px] leading-[18px] text-kumo-subtle">
                {isFiltered
                  ? 'Tente outro filtro ou termo de busca.'
                  : 'Tudo o que os seus espaços criarem aparece aqui.'}
              </p>
            </div>
            {/* Offer the deployment's formats here rather than sending them to the home page. */}
            {!isFiltered && <NewFormatRow label="Comece com" />}
          </div>
        ) : view === 'grid' ? (
          <div className="grid grid-cols-2 gap-4 px-3 sm:grid-cols-3 lg:grid-cols-4">
            {filtered.map((output) => (
      ...
      <DeleteConfirmationDialog
        ...
        description={
          <>
            Isso remove o arquivo de “{removeOutput?.workspaceTitle}” de forma permanente
            {removeOutput?.owner ? ', para todos com acesso a esse espaço' : ''}. Os outros
            arquivos desse espaço continuam disponíveis. Essa ação não pode ser desfeita.
          </>
        }
