...
function SectionLabel({ children }: { children: React.ReactNode }) {
  return (
    <h2 className="m-0 px-1 font-normal text-kumo-subtle">
...
export default function SettingsPage() {
  ...
  useEffect(() => {
    ...
    const fetchUserInfo = async () => {
      try {
        ...
        if (!cancelled) toasts.add({ title: 'Não foi possível carregar suas informações', variant: 'error' })
      } finally {
  ...
  const handleSaveName = async () => {
    if (!nameInput.trim()) {
      toasts.add({ title: 'O nome de exibição não pode ficar vazio', variant: 'error' })
      return
    }
    ...
    try {
      ...
      toasts.add({ title: 'Não foi possível atualizar o nome de exibição', variant: 'error' })
    }
  }
  ...
  const handleCopyId = async () => {
    ...
    try {
      ...
      toasts.add({ title: 'ID de usuário copiado', variant: 'success' })
    } catch {
      toasts.add({ title: 'Não foi possível copiar', variant: 'error' })
    }
  }
  ...
  const handleAvatarUpload = async (file: File) => {
    ...
    try {
      ...
      toasts.add({ title: 'Foto atualizada', variant: 'success' })
    } catch (err) {
      console.error('Failed to upload avatar:', err)
      setLocalAvatarPreview(null)
      toasts.add({ title: 'Não foi possível enviar a foto', variant: 'error' })
    } finally {
  ...
  const handleChangePassword = async () => {
    ...
    try {
      ...
      toasts.add({ title: 'Senha alterada', variant: 'success' })
      setCurrentPassword('')
      setNewPassword('')
      setConfirmPassword('')
    } catch (err) {
      const errorMessage = err instanceof Error ? err.message : 'Não foi possível alterar a senha'
  ...
  return (
    <div className="mx-auto flex h-full w-full max-w-2xl flex-col px-6 pb-16 sm:px-10">
      <PageHeader
        label="Perfil"
        title="Sua conta."
        description="Gerencie os dados da sua conta, sua foto e a segurança."
      />
        ...
        <section className="flex flex-col gap-3">
          ...
          <div className="divide-y divide-kumo-line overflow-hidden rounded-xl border border-kumo-line bg-kumo-base">
            ...
            <div className="flex items-center gap-4 px-5 py-4">
              ...
              <div className="min-w-0">
                ...
                <p className="mt-0.5 text-[12px] leading-4 tracking-[-0.2px] text-kumo-subtle">
                  Clique na foto para enviar uma nova
                </p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-2 px-5 py-4">
              <div className="min-w-0 flex-1">
                <FieldLabel>ID de usuário</FieldLabel>
              ...
              <button
                ...
                aria-label="Copiar ID de usuário"
                className={ICON_BTN}
              >
                <Copy size={14} />
              </button>
