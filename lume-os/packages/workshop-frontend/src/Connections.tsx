...
export default function Connections({ overseer, gadget, chatId, authenticatedApi, onConnectionsChange, isVisible, onHasGatekeepersChange }: ConnectionsProps) {
  ...
  const loadGatekeepers = async () => {
    try {
      ...
      toasts.add({ title: 'Não foi possível carregar as conexões', variant: 'error' })
    } finally {
      setLoading(false)
    }
  }
  ...
  const handleDeleteHookConfirm = async () => {
    ...
    try {
      ...
      toasts.add({ title: 'Não foi possível excluir o gatilho', variant: 'error' })
    } finally {
  ...
  const handleEditSave = async (name: string) => {
    ...
    if (!newName) {
      toasts.add({ title: 'O nome do vínculo não pode ficar vazio', variant: 'error' })
      return
    }
    if (newName === name) {
    ...
    try {
      ...
      toasts.add({ title: 'Não foi possível atualizar o nome do vínculo', variant: 'error' })
    } finally {
      setEditingBinding(null)
    }
  }
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      toasts.add({ title: 'Não foi possível remover a conexão', variant: 'error' })
    } finally {
  ...
  return (
    <div className="h-full overflow-auto bg-kumo-base">
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div className="min-w-0 flex-1">
                          <p className="flex items-center gap-2 truncate text-[13px] leading-[18px] font-medium tracking-[-0.25px] text-kumo-default">
                            ...
                            {isPending && (
                              <Tooltip content="Adicionado nesta conversa; fica quando você aceitar as alterações da conversa" asChild>
                        ...
                        <div className="ml-auto flex shrink-0 items-center gap-1">
                          <Tooltip content="Editar o nome usado no código" asChild>
                            <WorkshopIconButton
                              onClick={() => handleEditStart(gk.name)}
                              aria-label="Editar o nome usado no código"
                            >
                              <Pencil size={14} />
                            </WorkshopIconButton>
                          </Tooltip>
                          {!isPending && (
                            <Tooltip content="Editar configurações do modelo" asChild>
                              <WorkshopIconButton
                                onClick={() => setAnnotationTarget(gk)}
                                aria-label="Editar configurações do modelo"
                              >
                                <Blueprint size={14} />
                              </WorkshopIconButton>
                            </Tooltip>
                  ...
                  >
                    {isDeleting ? (
                      <div className="flex flex-wrap items-center gap-3">
                        <div className="min-w-0 flex-1">
                          <p className="truncate text-[13px] leading-[18px] font-medium tracking-[-0.25px] text-kumo-danger">
                            Excluir o gatilho "{hook.description.title}"?
                          </p>
                          <p className="truncate text-[12px] leading-4 font-normal tracking-[-0.2px] text-kumo-subtle">
                            Isso remove o gatilho de vez. Os próximos eventos deixarão de ser entregues.
                          </p>
                        </div>
                        <WorkshopButton
                          tone="danger"
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div className="ml-auto flex shrink-0 items-center gap-2">
                          ...
                          />
                          <Tooltip content="Excluir gatilho" asChild>
                            <WorkshopIconButton
                              danger
          ...
          try {
            ...
            toasts.add({
              title: chatId === undefined
                ? 'Conexão criada'
                : "Conexão criada: aceite as alterações da conversa para mantê-la",
              variant: 'success',
            })
            await loadGatekeepers()
            onConnectionsChange?.()
      ...
      <BlueprintAnnotationModal
        ...
        onSaved={() => {
          toasts.add({ title: 'Configurações do modelo salvas.', variant: 'success' })
    ...
    ;(async () => {
      try {
        ...
        if (!cancelled) {
          ...
          setLoadError(err?.message || 'Não foi possível carregar o vínculo.')
        }
      }
    })()
    return () => {
          ...
          <div className="flex items-start justify-between gap-4 border-b border-kumo-line px-4 py-4 sm:px-5">
            <div className="min-w-0">
              ...
              <Dialog.Description className="mt-1 text-[12px] leading-4 font-normal tracking-[-0.2px] text-kumo-subtle">
                Como esta conexão aparece nos modelos.
              </Dialog.Description>
            </div>
            <Dialog.Close
              render={(props) => (
