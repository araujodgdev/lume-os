...
The deployment is six Workers. A **router** owns the public route and serves the frontend, proxying `/api` to the Workshop backend and `/gatekeeper/<name>` to whichever Gatekeeper the binding name matches; the Workshop, the Context, Scheduler and Casos Gatekeepers (the Casos Worker also serves the Agenda and Pesquisa), and the Error Reporter sit behind it with no route of their own, reachable only over service bindings.
...
- Open `/admin`, confirm the email is an administrator, and set the Context, Scheduler, Casos, Agenda and Pesquisa Gatekeepers to disabled, optional, or enabled. Set Casos, Agenda and Pesquisa to enabled so every lawyer gets the case registry, the firm's calendar and case-law research.
