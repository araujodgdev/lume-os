...
The deployment is six Workers. A **router** owns the public route and serves the frontend, proxying `/api` to the Workshop backend and `/gatekeeper/<name>` to whichever Gatekeeper the binding name matches; the Workshop, the Context, Scheduler and Casos Gatekeepers (the Casos Worker also serves the Agenda), and the Error Reporter sit behind it with no route of their own, reachable only over service bindings.
...
- Open `/admin`, confirm the email is an administrator, and set the Context, Scheduler, Casos and Agenda Gatekeepers to disabled, optional, or enabled. Set Casos and Agenda to enabled so every lawyer gets the case registry and the firm's calendar.
