...
- **Added:** Intimações, a fourth vendor in the Casos Worker (`ProcessosVendor`, bound as `GATEKEEPER_PROCESSOS`). It follows the firm's cases in the courts: pending PJe notices through each lawyer's MNI access (CPF and PJe password, encrypted with the Worker secret `LUME_CHAVE_CREDENCIAIS`, which the deploy generates once), DJEN publications by OAB number and DataJud docket entries. It never opens a PJe notice by itself. New items become notifications, suggested deadlines in the Agenda and, optionally, an agent summary.
...
- Nunca abra uma intimação do PJe: abrir registra a ciência e começa o prazo, e só o advogado faz isso, na página Intimações. Leia o que chegou com `PROCESSOS.intimacoes()` e `PROCESSOS.movimentacoes()`. Ao resumir, destaque o que foi decidido, o que fazer e os riscos; diga quais intimações ainda estão fechadas e quando ocorre a ciência tácita; trate os prazos sugeridos na Agenda como a confirmar.
