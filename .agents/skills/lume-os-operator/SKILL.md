---
name: lume-os-operator
description: Guides araujodgdev/lume-os setup, Cloudflare Access, deployment.jsonc, storage, AI, observability, custom Gatekeepers, deployment, verification, troubleshooting, rollback, and pinned-submodule upgrades. Use only for deployments created from the Lume OS Deployment Starter, not standalone Lume OS checkouts.
...
This skill operates the deployment wrapper, not Lume OS in isolation. The wrapper vendors a pinned Lume OS source tree in `lume-os/`, derives temporary Wrangler configs, and coordinates several Workers.
...
Do not assume the current working directory or this skill's directory is the starter root. If the markers are absent, stop and ask for the starter checkout. Do not apply this workflow directly to a standalone Lume OS checkout.

Once the submodule is initialized, confirm `lume-os/packages/router/` exists as well. The deploy binds and deploys it, so a pinned commit without it cannot be deployed by this wrapper; treat its absence as a provenance problem, not a missing build step.
...
4. `package.json`
5. `.gitmodules`
6. Available relevant upstream files under `lume-os/`
...
- **Upgrade:** move the pinned `lume-os` gitlink to a reviewed commit.
