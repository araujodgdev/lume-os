# Lume OS Starter Troubleshooting
...
| --- | --- | --- |
| `lume-os/package.json` is absent | `git submodule status`, parent gitlink, `.gitmodules` | Initialize only from the reviewed configured source. |
...
| Package missing under submodule | Separate root and `lume-os` installs | Run both documented installs. |
